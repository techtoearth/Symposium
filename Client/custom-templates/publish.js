...
function buildNav(members) {
    let globalNav;
    let nav = '<h2><a href="index.html">Client</a></h2>';
    const seen = {};
    const seenTutorials = {};
    ...
    if (members.globals.length) {
        ...
        if (!globalNav) {
            ...
            nav += `<h3>${linkto('global', 'Modules')}</h3>`;
        }
        else {
            nav += `<h3>Modules</h3><ul>${globalNav}</ul>`;
        }
    }
...
exports.publish = (taffyData, opts, tutorials) => {
    ...
    if (members.globals.length) { generate('Modules', [{kind: 'globalobj'}], globalUrl); }
